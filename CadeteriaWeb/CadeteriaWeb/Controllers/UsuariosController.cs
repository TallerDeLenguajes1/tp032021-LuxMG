...
namespace CadeteriaWeb.Controllers
{
    public class UsuariosController : SessionController
    {
        ...
        public IActionResult Register(UsuarioCreateViewModel usuarioVM = null)
        {
        ...
        public IActionResult CreateUsuarioPost(UsuarioCreateViewModel usuario)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        usuario.Message = "Ya existe un usuario con ese nombre";
                        return RedirectToAction("CreateUsuario", usuario);
                    }
        ...
        public IActionResult UpdateUsuario(int id = 0, UsuarioUpdateViewModel usuarioVM = null)
        {
            if (!IsSesionIniciada())
                return RedirectToAction("Login");
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if ((DB.Usuarios.GetUsuarioByName(U.Username) != null) && (DB.Usuarios.GetUsuarioByName(U.Username)?.Id != U.Id))
                    {
                        ...
                        return RedirectToAction("UpdateUsuario", usuarioVM);
                    }
                ...
                return RedirectToAction("UpdateUsuario", usuario); // si hay un error en el ModelState
            }
            catch (Exception e)
            {
                _logger.LogError($"ERROR EN UPDATE DE USUARIO - EXCEPTION:{e.Message}");
